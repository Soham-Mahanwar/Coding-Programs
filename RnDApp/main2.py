...
def play_music(path=None):
    try:
        pygame.mixer.init()
        mp = path if path else MUSIC_FILE
        if os.path.exists(mp):
            pygame.mixer.music.load(mp)
            pygame.mixer.music.play(-1)  # loop forever
        else:
            print(f"[Warning] Music file '{mp}' not found.")
...
class FocusMateApp:
    def __init__(self, root):
        ...
        # music file chosen by user (None means use default MUSIC_FILE)
    ...
    def show_start_screen(self):
        ...
        tk.Label(frame, text="🌙 FocusMate",
                 font=("Helvetica", 48, "bold"), fg="#00FFFF", bg="#000814").pack(pady=30)

        tk.Label(frame, text="Smart Work–Break Assistant",
                 font=("Helvetica", 24), fg="#00FFAA", bg="#000814").pack(pady=5)

        # Work & Break duration input
        tk.Label(frame, text="Work Duration (minutes):", fg="white", bg="#000814", font=("Helvetica", 16)).pack(pady=(20, 5))
        work_entry = tk.Entry(frame, font=("Helvetica", 16), justify="center", width=10)
        work_entry.insert(0, str(DEFAULT_WORK_DURATION // 60))
        work_entry.pack()

        tk.Label(frame, text="Break Duration (minutes):", fg="white", bg="#000814", font=("Helvetica", 16)).pack(pady=(10, 5))
        ...
        music_frame = tk.Frame(frame, bg="#000814")
        music_frame.pack(pady=(15,5))
        self.music_label = tk.Label(music_frame, text=f"Break music: {os.path.basename(self.music_file) if self.music_file else os.path.basename(MUSIC_FILE)}",
                                    fg="white", bg="#000814", font=("Helvetica", 12))
        self.music_label.pack(side="left", padx=10)

        tk.Button(music_frame, text="Choose Break Music", command=self.choose_music,
                  font=("Helvetica", 10), bg="#333333", fg="white").pack(side="left")
        ...
        tk.Button(frame, text="Start Work Session", command=start_session,
                  font=("Helvetica", 20, "bold"), bg="#00AAFF", fg="white", width=22).pack(pady=20)

        tk.Button(frame, text="Change Password", command=self.show_change_password_screen,
                  font=("Helvetica", 16), bg="#111111", fg="white", width=20).pack(pady=8)

        tk.Button(frame, text="Exit", command=self.root.destroy,
                  font=("Helvetica", 16), bg="#FF3333", fg="white", width=20).pack(pady=8)

    # ---------------- choose music ----------------
    def choose_music(self):
        file_path = filedialog.askopenfilename(
            title="Select Break Music",
            filetypes=[("Audio files", "*.mp3 *.wav *.ogg"), ("All files", "*.*")]
        )
        if file_path:
            self.music_file = file_path
            self.music_label.config(text=f"Break music: {os.path.basename(self.music_file)}")
            messagebox.showinfo("Music Selected", "Break music has been set successfully!")
    ...
    def show_password_setup_screen(self):
        ...
        tk.Button(frame, text="Save & Continue", command=set_password,
                  font=("Helvetica", 18, "bold"), bg="#00AAFF", fg="white").pack(pady=20)
    ...
    def show_change_password_screen(self):
        ...
        tk.Label(frame, text="🔑 Change Password",
                 font=("Helvetica", 28, "bold"), fg="#00FFFF", bg="#000814").pack(pady=30)
        ...
        tk.Button(frame, text="Change Password", command=change_pw,
                  font=("Helvetica", 18, "bold"), bg="#00AAFF", fg="white").pack(pady=12)

        tk.Button(frame, text="Back", command=self.show_start_screen,
                  font=("Helvetica", 14), bg="#222", fg="white").pack(pady=6)

    # ---------------- WORK PHASE ----------------
    def start_work_phase(self):
        # small floating window for work
    ...
    def start_break_phase(self):
        ...
        # Play chosen music if set, else default
        play_music(self.music_file if self.music_file else None)

        self.exercise_index = 0
        # avoid division by zero
        if len(exercises) > 0:
            self.exercise_duration = max(1, self.break_duration // len(exercises))
        else:
            self.exercise_duration = self.break_duration
    ...
    def show_password_window(self):
        # Ensure dialog is on top of fullscreen
        # create a transient dialog using simpledialog-style Toplevel
        ...
        def check_password():
            if verify_password(pw_entry.get()):
                ...
                self.running = False
                # Close the fullscreen break frame and go back to start screen
                self.show_start_screen()
            else:
                feedback_label.config(text="❌ Incorrect password! Try again.")

        def on_close():
            # simply destroy dialog (break remains)
            pw_window.destroy()
    ...
    def countdown(self, duration, callback):
        def update(remaining=duration):
            mins, secs = divmod(remaining, 60)
            self.timer_label.config(text=f"{mins:02}:{secs:02}")
            if remaining > 0 and self.running:
                self.root.after(1000, lambda: update(remaining - 1))
            else:
                callback()
        update()
